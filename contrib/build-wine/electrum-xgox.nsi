...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-XGOX"
  !define PRODUCT_WEB_SITE "https://github.com/ddude1/electrum-xgox"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-xgox-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-xgox.ico"
...
Section
  ...
  File /r "dist\electrum-xgox\*.*"
  File "icons\electrum-xgox.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe" 0


  ;Links xgox: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\xgox" "" "URL:xgox Protocol"
  WriteRegStr HKCU "Software\Classes\xgox" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\xgox" "DefaultIcon" "$\"$INSTDIR\electrum-xgox.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\xgox\shell\open\command" "" "$\"$INSTDIR\electrum-xgox-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-xgox.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\xgox"
