...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias xgoxcoin.com -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile xgox-electrum \
        -keystore ~/.jks/keystore \
        Electrum_XGOX-3.0.6.1-release-unsigned.apk \
        xgoxcoin.com
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_XGOX-3.0.6.1-release-unsigned.apk \
        Electrum_XGOX-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Electrum-XGOX-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Electrum-XGOX-{version}'
PPA_SOURCE_NAME = 'electrum-xgox'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_XGOX-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Electrum_XGOX-{version}-release.apk'
